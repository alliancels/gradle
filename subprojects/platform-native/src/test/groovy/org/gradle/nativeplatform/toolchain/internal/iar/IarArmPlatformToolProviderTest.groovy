...
class IarArmPlatformToolProviderTest extends Specification {
    def "windows shared link file names end with lib"() {
        given:
        def operatingSystem = Mock(OperatingSystemInternal)
        IarArmPlatformToolProvider toolProvider = new IarArmPlatformToolProvider(Mock(BuildOperationProcessor), operatingSystem, [:], Mock(IarArmInstall), Mock(NativePlatformInternal), Mock(ExecActionFactory) )
